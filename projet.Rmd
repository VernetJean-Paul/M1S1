...
```{r}
df %>%
  select(name, popularity, start_date) %>%
  distinct() %>%
  arrange(popularity) %>%
  slice(1:10) %>%
  separate(col = start_date, into = c("years", "month"), convert = T) %>%
  arrange(-years) %>%
  ggplot(aes(x= years, y = name, colour = name)) +
  geom_point(show.legend = F) +
  labs(title = "Apparition des 10 animés les plus populaires dans le temps",
       y = "Nom des animés",
       x = "Année")
